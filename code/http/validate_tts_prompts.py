...
#
# Runs AFTER Stage 5 (VOPlan.{locale}.json produced) and BEFORE
# gen_tts_cloud.py is called in Stage 9.
#
# Called by run.sh in Stage 9 for each locale before gen_tts_cloud.py.
#
# Reads:
#   VOPlan.{locale}.json         — locale manifest with tts_prompt per item
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "Validate that tts_prompt fields in VOPlan.{locale}.json\n"
...
def main() -> None:
    ...
    if not manifest_path.exists():
        print(f"[WARN] VOPlan.{locale}.json not found — skipping")
        sys.exit(0)
