...
def main():
        ...
        for loc_dir in sorted(os.listdir(renders_root)):
            ...
            for fname in sorted(os.listdir(loc_render)):
                if not fname.endswith(".srt"):
                    continue
                # Map SRT locale suffix → YouTube language code + display name.
                # All 4 locales generated by run_sgf.py / _pipe_generate_srts() are covered.
                _SRT_LOCALE_MAP = {
                    ".en.":      ("en",    "English"),
                ...
                for suffix, (lang_code, lang_name) in _SRT_LOCALE_MAP.items():
                    if suffix in fname:
                        subtitles.append({"file": f"renders/{loc_dir}/{fname}",
                                          "language": lang_code, "name": lang_name})
                        break
    ...
    # --locale argument always wins; profile locale is only a fallback when --locale is absent.
    # Previously profile_info.get("locale") took priority, which caused zh-Hans runs on the
    # 'games' profile (locale="en") to generate English titles/descriptions.
    _eff_locale = locale or profile_info.get("locale")
    ...
    # ── Locale validation: retry if Chinese expected but title is non-CJK ─────
    def _has_cjk(text: str) -> bool:
        """Return True if text contains at least one CJK ideograph (U+4E00-U+9FFF or U+3400-U+4DBF)."""
        return any(
    ...
    if output_lang.startswith("Chinese") and not _has_cjk(suggested.get("title", "")):
        _bad_title = suggested.get("title", "")
        print(f"  ⚠ title is non-Chinese despite locale={locale!r}: {_bad_title!r}", file=sys.stderr)
        print("  Retrying with reinforced Chinese instruction…", file=sys.stderr)
        _reinforced = (
            "CRITICAL CORRECTION: Your previous response wrote the title in English. "
            "You MUST write the title, description, and tags in Chinese (Simplified, 普通话) only. "
            "The title MUST contain Chinese characters — no English title is acceptable. "
            "Do NOT start the title with English words.\n\n"
        ) + prompt_text
        _raw_retry = _call_claude(_reinforced, PIPE_DIR)
        _sug_retry, _err_retry = _try_parse(_raw_retry)
        if _sug_retry is not None and not (REQUIRED - set(_sug_retry.keys())):
            if not _has_cjk(_sug_retry.get("title", "")):
                print(f"  ⚠ retry also non-Chinese ({_sug_retry.get('title')!r}) — using retry anyway", file=sys.stderr)
            else:
                print(f"  ✓ retry produced Chinese title: {_sug_retry.get('title')!r}", file=sys.stderr)
            suggested = _sug_retry
        else:
            print(f"  ⚠ retry unusable ({_err_retry}) — keeping original (non-Chinese) result", file=sys.stderr)
